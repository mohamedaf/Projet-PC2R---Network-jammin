...
public class EchoServer {
	 ...
	 */
	private HashMap<Integer, ArrayList<float[]>> hashBuffers;
	...
	public EchoServer(int port, int capacity) {
		...
		this.serv = null;
		this.serv2 = null;
		this.hashBuffers = new HashMap<Integer, ArrayList<float[]>>();
	...
	public byte[] AnswerJamClient(String s, PrintWriter out, EchoJamClient cl) {
		...
		System.out.println("commande recue : " + s);
		System.out.flush();
	 ...
	 * @return : le buffer envoye par le client
	 */
	public float[] AnswerJamClient2(String s, PrintWriter out, EchoJamClient cl) {
		float[] buffer = null;
		Integer tick;

		/** Traitement reception buffer audio */
		s = Commandes.filter(s);
		...
		if (tab[0].contains("AUDIO_CHUNK") && tab.length == 3
				&& (!tab[1].equals("")) && (!tab[2].equals(""))) {
			...
			/** On met a jour la variable representant le tick actuel */
			if (tick > this.getTickActuel())
				this.setTickActuel(tick);

			/** recuperer les donnees dans le buffer */
			buffertmp = tab[2].split(" ");
			setSizeBuff(buffertmp.length);
			buffer = new float[this.getSizeBuff()];

			/** Convetir les donnees recues en int puis en float */
			for (int i = 0; i < this.getSizeBuff(); i++) {
				buffer[i] = (float) (Integer.parseInt(buffertmp[i]) / 32767.0);
	...
	public void run() {
		try {
			...
			ds.start();
			System.out.println("Please type \"exit\" to quit proprely");
	 ...
	 * @return : hashBuffers
	 */
	public HashMap<Integer, ArrayList<float[]>> getHashBuffers() {
		return hashBuffers;
	}
